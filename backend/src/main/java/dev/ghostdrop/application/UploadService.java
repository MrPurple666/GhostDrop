...
public final class UploadService {
    ...
    public CreateUploadResult create(CreateUploadRequest request) {
        ...
        var file =
                new TemporaryFile(
                        ...
                        blankToNull(request.password()) == null
                                ? null
        ...
        return new CreateUploadResult(
                id,
                storage.createUploadUrl(storageKey, file.contentType(), file.fileSize()),
                "/d/" + id,
                deletionToken,
                file.expiresAt());
    }
    ...
    private void validate(CreateUploadRequest request) {
        if (request == null
                || blankToNull(request.fileName()) == null
                || request.fileName().length() > 255
                || blankToNull(request.contentType()) == null
                || request.fileSize() < 1
                || request.fileSize() > settings.maximumFileSizeBytes()
                || request.expiresInSeconds() < settings.minimumLifetimeSeconds()
                || request.expiresInSeconds() > settings.maximumLifetimeSeconds()
                || request.maxDownloads() != null && request.maxDownloads() < 1) {
